...
#define SLEEP_IS_ACTIVE ENABLE_DEEP_SLEEP
...
#define NUM_OF_SCD41_REG_VARS 8
...
,
    { &def_table_version, NULL, NULL, (u8*)&scd41_reg->vars.table_version, 102,"table version, resets regs to defaults on mismatch","table_version", 0x14016007, 0x312f8, U32_REGS_FLAG, 1, 7, 11 }//!< "table version, resets regs to defaults on mismatch" &ro &save &def
,
};
...
static int store_array_of_co2_levels(uint16_t co2_level);
static int scd41_task_init(void){
   int result = 0;
   regs_global->vars.current_state[0] |= CS0_TASK_ACTIVE_SCD41;
   int table_ind = link_functions.regs_description_list_add_new(regs_table_scd41_reg);
