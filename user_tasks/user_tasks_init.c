...
#if STEP_MOTOR
    res = link_functions.os_thread_create(step_motor_task, "step_motor_task", 2464, NULL, (tskIDLE_PRIORITY + 2), &step_motor_handle_id);
    if (res != pdTRUE) {
...
#if FEEDER
    res = link_functions.os_thread_create(feeder_task, "feeder_task", 2464, wakeup_cause, (tskIDLE_PRIORITY + 2), &feeder_handle_id);
    if (res != pdTRUE) {
...
#if POLISHER
    res = link_functions.os_thread_create(polisher_task, "polisher_task", 2464, wakeup_cause, (tskIDLE_PRIORITY + 2), &polisher_handle_id);
    if (res != pdTRUE) {
...
#if TEST_INT
    res = link_functions.os_thread_create(test_int_task, "test_int_task", 2464, wakeup_cause, (tskIDLE_PRIORITY + 2), &test_int_handle_id);
    if (res != pdTRUE) {
...
#if EPAPER
    res = link_functions.os_thread_create(epaper_task, "epaper_task", 2464, wakeup_cause, (tskIDLE_PRIORITY + 2), &epaper_handle_id);
    if (res != pdTRUE) {
...
#if PWM_CONTROL_ENABLE
    res = link_functions.os_thread_create(pwm_control_task, "pwm_control_task", 2048, NULL, (tskIDLE_PRIORITY + 2), &pwm_task_handle);
    if (res != pdTRUE) {
